...
#define M_E 2.7182818284590452354     // euler's constant for sigmoid function
...
void setup() {
  Serial.begin(9600);
  Serial.println(F("*** Executing WHS_v2.1.1.ino ***"));
  ...
  initial_distance = dist[window-1];
...
void loop() {
  ...
  distance = sea_level + (initial_distance - filtered);
  ...
  float busvoltage = 0;
  float power_mW = 0;
  busvoltage = ina219.getBusVoltage_V() + 0.8;    // add 0.8V to account for the drop across the diode
    ...
    if(prevDist != -1) {
      ...
      if(prevFirstDeriv >= 0) {
        ...
        // the array is fully populated, shift everything over one index
          ...
          sampling_rate = changeTime(avgSecondDeriv);
        }
      }
  ...
  int remainder = sampling_rate % keepAlive_mins;
...
//    }
  ...
  Serial.print(F("Waiting for ")); Serial.print(remainder); Serial.println(F(" minutes\r\n"));
  digitalWrite(yellowLed, LOW);
...
float changeTime(float avgSecondDeriv) {
